...
use crate::proxy::stmt_shadow::{PgStmtShadow, scan_pg_pipeline};
...
impl SessionQueryTracker {
    ...
    fn summarise(&self) {
        ...
        for (_, fp, count) in &repeated {
            log::info!("[pg conn {}] repeated query x{}: {}", self.conn_id, count, fp);
        }
    }
}
...
impl PgProxyServer {
    pub fn new(
        ...
        let protocol: Arc<dyn DatabaseProtocol> = if !config.ssl_cert.is_empty() && !config.ssl_key.is_empty() {
            ...
            match crate::protocol::mysql::tls::build_frontend_acceptor(&tls_cfg) {
                Ok(acceptor) => {
                    ...
                    Arc::new(crate::protocol::PostgreSQLProtocol::new_with_tls(users.clone(), acceptor))
                }
                Err(e) => {
                    log::warn!("[pg] Failed to build TLS acceptor ({}), falling back to plain", e);
        ...
        let idle = if config.connection_max_idle_secs == 0 { None }
                   else { Some(Duration::from_secs(config.connection_max_idle_secs)) };
        ...
        let max_conn = if config.max_connections == 0 { 10_000 } else { config.max_connections };
    ...
    pub fn pool(&self) -> Arc<BackendPool> { self.pool.clone() }
    ...
    pub async fn run(self: Arc<Self>) -> anyhow::Result<()> {
        let listener = TcpListener::bind(&self.config.listen_addr)
            ...
            .map_err(|e| anyhow::anyhow!(
                "Failed to bind PostgreSQL listener to {} — {}", self.config.listen_addr, e
            ))?;
        ...
        loop {
            let (socket, addr) = match listener.accept().await {
                Ok(v) => v,
                Err(e) => { log::warn!("[pg] accept error: {}", e); continue; }
            };
...
async fn handle_pg_connection(
    ...
    let client_database = {
        ...
        if db.is_empty() { "postgres".to_string() } else { db.to_string() }
    };
    log::debug!("[pg conn {}] accepted user='{}' from {}", conn_id, username, client_ip);

    // Per-user init: search_path, isolation level
    let user_cfg = srv.users_config.iter().find(|u| u.name == username).cloned();
    ...
    if !session_init_sqls.is_empty() {
        if let Ok(mut conn) = srv.pool.get_primary_for_database(Some(&client_database)).await {
            for sql in &session_init_sqls {
                let _ = conn.execute_query(sql.as_bytes()).await;
            }
            srv.router.put_primary_for_database(conn, &client_database).await;
        }
    }
        ...
        match cmd {
            ...
            Command::Query(sql) => {
                ...
                if sql_str.starts_with('\\') {
                    match translate_meta_command(sql_str) {
                        Some(translated_sql) => {
                            ...
                            let result = srv.router.route_query_with_database(
                                &translated_sql,
                                &mut tx_conn,
                            ...
                            match result {
                                Ok(r) => { let _ = session.write_response(&r.bytes).await; }
                                Err(e) => { let _ = session.write_error("XX000", &e.to_string()).await; }
                            }
                ...
                if !session.allow_writes() && matches!(intent, QueryIntent::Write) {
                    let _ = session.write_error("42501", "User is not allowed to write (read-only account)").await;
                    let _ = session.flush().await;
                    continue;
                }
                ...
                if !srv.query_whitelist.is_allowed(sql_str) {
                    srv.metrics.whitelist_blocked.fetch_add(1, Ordering::Relaxed);
                    log::warn!("[pg conn {}] whitelist blocked user='{}': {}", conn_id, username, &sql_str[..sql_str.len().min(120)]);
                    srv.audit_logger.log(&username, &client_ip, sql_str, "blocked:whitelist", 0.0, true);
                    srv.error_events.push(ErrorEvent::new_pg(
                        "Query not permitted: not in the PostgreSQL query allowlist",
                        fingerprint(sql_str),
                        &client_ip, &username, 0.0,
                    ));
                    let _ = session.write_error("42501", "Query not permitted: not in the query allowlist").await;
                    let _ = session.flush().await;
                    continue;
                }
                ...
                if starts_tx { session.set_in_transaction(true); }
                if starts_tx {
                    active_trace = Some(ActiveTrace::new(conn_id, &username, &client_ip));
                }

                // ── RYOW ───────────────────────────────────────────────────
                let ryow_active = ryow_ms > 0 && ryow_until.map_or(false, |t| t > Instant::now());
                ...
                if ends_tx {
                    ...
                    if let Some(conn) = tx_conn.take() { srv.router.put_primary_for_database(conn, &client_database).await; }
                } else if let Some(status) = result.as_ref().ok()
                ...
                match result {
                    Ok(response) => {
                        if response.is_error {
                            log::debug!("[pg conn {}] backend error for '{}'", conn_id, &sql_str[..sql_str.len().min(100)]);
                        } else {
                            let fp = fingerprint(sql_str);
                            srv.collector.try_record(sql_str, elapsed, matches!(intent, QueryIntent::Read));
                            srv.audit_logger.log(&username, &client_ip, sql_str, "primary", elapsed.as_secs_f64() * 1000.0, false);
                            ...
                            if slow_ms > 0 && elapsed.as_millis() as u64 >= slow_ms {
                                log::warn!("[pg slow {}ms] {}", elapsed.as_millis(), &fp[..fp.len().min(200)]);
                            }
                            // RYOW window
                            ...
                            if let Some((fp, example_sql, count)) = query_tracker.track_exact(sql_str) {
                                srv.regression_store.report_hot_key(&fp, &example_sql, count);
                            }
                            ...
                            if let Some(ref mut trace) = active_trace {
                                ...
                                trace.record(sql_str, fp, elapsed.as_secs_f64() * 1000.0, &backend_addr, intent_str);
                            }
                        }
                    ...
                    Err(e) => {
                        ...
                        let sqlstate = if is_gone && session.is_in_transaction() {
                            ...
                            session.set_in_transaction(false);
                            if let Some(conn) = tx_conn.take() { drop(conn); }
            ...
            Command::Stmt(raw) => {
                ...
                let t0 = Instant::now();
                let result = srv.router.route_stmt_pg_shadow_with_database(
                    &raw,
                    &scan,
                    &pg_shadow,
                    &mut stmt_conn,
                ...
                match result {
                    Ok(response) => {
                        ...
                        for p in &scan.parses {
                            ...
                            srv.collector.try_record(q, elapsed, matches!(q_intent, QueryIntent::Read));
                        ...
                        srv.throughput.record((elapsed.as_secs_f64() * 1_000_000.0) as u64);
                        if let Err(e) = session.write_response(&response.bytes).await {
                    ...
                    Err(e) => {
                        ...
                        srv.error_events.push(ErrorEvent::new_pg(
                            err_str.clone(), "extended_query", &client_ip, &username,
                            elapsed.as_secs_f64() * 1000.0,
                        ));
            ...
            Command::Other(raw) => {
                ...
                let mut conn = match srv.pool.get_primary_for_database(Some(&client_database)).await {
                    Ok(c)  => c,
                    Err(e) => {
                        srv.copy_active.fetch_sub(1, Ordering::Relaxed);
                        log::warn!("[pg conn {}] get primary: {}", conn_id, e);
                        let _ = session.write_error("08006", "could not connect to primary").await;
                        let _ = session.flush().await;
                        break;
                    }
                };
    ...
    if let Some(conn) = tx_conn   { srv.router.put_primary_for_database(conn, &client_database).await; }
    if let Some(conn) = stmt_conn { srv.router.put_primary_for_database(conn, &client_database).await; }
