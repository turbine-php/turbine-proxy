...
impl PgHealthChecker {
    ...
    async fn check_primary(&self) {
        ...
        if ok {
            let prev = self.pool.primary_health.consecutive_failures.swap(0, Ordering::Relaxed);
            ...
            if was_down || prev >= self.failover_threshold {
                ...
                if had_failover {
                    log::info!("[pg health] Primary {} recovered — failover cleared", self.primary_config.addr);
                }
            }
        } else {
    ...
    async fn ping_and_check_primary(&self, config: &BackendConfig) -> bool {
            ...
            match tokio::time::timeout(
                ...
                Ok(Ok(resp)) if resp.status().is_success() => {
                    if let Ok(json) = resp.json::<serde_json::Value>().await {
                        ...
                        if !is_leader {
                            log::warn!("[pg health] Patroni reports primary {} as role='{}' (not leader)", config.addr, role);
                            return false;
                        }
                    }
                }
        ...
        match self.protocol.connect_backend(config).await {
            Err(_) => false,
            Ok(mut conn) => {
                if conn.ping().await.is_err() { return false; }
                // `pg_is_in_recovery()` must return 'f' for a writable primary
                match conn.execute_query(b"SELECT pg_is_in_recovery()").await {
                    Ok(resp) => !resp.is_error && !response_contains(&resp.bytes, b"true") && !response_contains(&resp.bytes, b"t"),
                    Err(_) => false,
                }
            }
        }
    ...
    async fn check_replica(&self, idx: usize, config: &BackendConfig) {
        if idx >= self.pool.replica_health.len() { return; }
        ...
        if self.patroni_check {
            ...
            match tokio::time::timeout(
                ...
                Ok(Ok(resp)) if resp.status().is_success() => {
                    if let Ok(json) = resp.json::<serde_json::Value>().await {
                        let role = json["role"].as_str().unwrap_or("");
                        let is_replica = role == "replica" || role == "standby" || role == "standby_leader";
                        if !is_replica {
                            log::warn!("[pg health] Patroni reports replica [{}] {} as role='{}' (not standby)", idx, config.addr, role);
                            self.pool.replica_health[idx].healthy.store(false, Ordering::Relaxed);
                            self.pool.replica_health[idx].consecutive_failures.fetch_add(1, Ordering::Relaxed);
                            return;
                        }
                    }
                }
        ...
        match self.protocol.connect_backend(&control_cfg).await {
            Err(e) => {
                ...
                if was_healthy || failures == 1 {
                    log::warn!("[pg health] Replica [{}] {} unreachable: {}", idx, control_cfg.addr, e);
                }
            }
            Ok(mut conn) => {
                // Confirm this is actually a standby
                let in_recovery = match conn.execute_query(b"SELECT pg_is_in_recovery()").await {
                    Ok(r) => !r.is_error && (response_contains(&r.bytes, b"t") || response_contains(&r.bytes, b"true")),
...
fn extract_text_value(bytes: &[u8]) -> Option<String> {
    ...
    while pos + 5 <= bytes.len() {
        ...
        if t == b'D' {
            ...
            fp += 4;
            if field_len < 0 { return None; } // NULL
            let flen = field_len as usize;
            if fp + flen > payload.len() { break; }
